🔴 1. FĀZE — Tailwind v4 migrācija (CSS-first config)
Problēma: Tailwind v4 ir instalēts, bet globals.css joprojām izmanto v3 sintaksi (@tailwind base/components/utilities), un tailwind.config.ts izmanto v3-stila JS konfigurāciju. Tailwind v4 pieprasa CSS-first pieeju.
#	Uzdevums	Detaļas
1.1	Migrēt globals.css importus	Nomainīt @tailwind base; @tailwind components; @tailwind utilities; → @import "tailwindcss";
1.2	Pārnest tailwind.config.ts uz CSS	Tailwind v4 konfigurācija notiek CSS ar @theme { } bloku. Pārvietot colors, borderRadius, boxShadow definīcijas no JS uz @theme bloku globals.css
1.3	Noņemt tailwind.config.ts	Pēc migrācijas šis fails vairs nav nepieciešams (vai atstāt tukšu ar @config atsauci, ja izmanto plugins)
1.4	Pārbaudīt @layer blokus	Tailwind v4 mainījis @layer uzvedību — pārliecināties, ka @layer base un @layer components joprojām darbojas
1.5	Build tests	next build + vizuāla pārbaude, ka visi 5 tēmu tokeni darbojas
...
#	Uzdevums	Detaļas
8.1	BYOK paplašināšana	Šobrīd tikai Kitchen AI. Pievienot AI palīdzību: Finance ieteikumi, Reset labsajūtas analīze
8.2	AI atslēgu pārvaldība	Settings jau atbalsta OpenAI + Gemini atslēgas — paplašināt moduli
1. FĀZE (Tailwind v4 migrācija)    ← VISPIRMS — tehnoloģiskais pamats
...
├── next.config.ts
├── next-env.d.ts
├── tailwind.config.ts                # ⚠️ V3-stila config → migrēt uz CSS @theme
...
└── src/
    ...
    │       ├── openai/test/route.ts            # OpenAI atslēgas tests
...
| **BYOK atslēgas nekad nenonāk serverī storage** | Ierīces atslēgas: `localStorage`. Household atslēgas: Supabase kolonna ar RLS (tikai biedri). API routes: atslēga nāk no DB per request, nekad netiek logota. |
