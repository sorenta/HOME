...
create table if not exists public.reset_wellness_goals (
  ...
  last_slip_reason text,
  body_mode text check (body_mode in ('weight_loss', 'bulk', 'lean')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

alter table public.reset_wellness_goals add column if not exists last_slip_at timestamptz;
alter table public.reset_wellness_goals add column if not exists last_slip_reason text;
...
alter table public.reset_wellness_goals force row level security;
alter table public.reset_body_measurements force row level security;
alter table public.reset_weigh_ins force row level security;
alter table public.reset_training_state force row level security;
alter table public.household_water_logs force row level security;
alter table public.household_water_medals force row level security;
...
drop policy if exists "household_water_logs_self_select" on public.household_water_logs;
...
drop policy if exists "household_water_medals_self_select" on public.household_water_medals;
...
create policy "household_water_logs_self_select" on public.household_water_logs
  for select using (user_id = auth.uid());
...
create policy "household_water_medals_self_select" on public.household_water_medals
  for select using (user_id = auth.uid());
