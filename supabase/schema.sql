...
create table if not exists public.notification_preferences (
  ...
  reset_empathy_recipient_ids uuid[] not null default '{}'::uuid[],
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (user_id)
);
...
alter table public.reset_checkins force row level security;
alter table public.reset_metrics force row level security;
