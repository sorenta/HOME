...
async function callVertexAI(system: string, user: string) {
  ...
  try {
    ...
    // Priority models for Vertex AI (using latest Gemini 3.1)
...
export async function POST(request: Request) {
  try {
    ...
    // Check if we should use DEVELOPER CREDITS (Vertex AI)
...
Do not invent data. Use only the provided information.`;
    ...
    if (useDeveloperCredits) {
      console.log(`[AI] Using Vertex AI Developer Credits for user: ${user.id}`);
