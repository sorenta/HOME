...
import { loadWellnessState, saveWellnessState, type ResetWellnessV1, type ResetTrackMetric } from "@/lib/reset-wellness";
...
const OnboardingRobotCanvas = dynamic(
  () => import("./onboarding-robot").then(mod => mod.OnboardingRobotCanvas),
  { ssr: false }
);
...
export function GlobalOnboarding({ onComplete }: { onComplete: () => void }) {
  ...
  const content = (
    <div className={`fixed inset-0 z-[999999] flex flex-col px-4 sm:px-8 transition-all duration-700 ${
      step === "welcome" ? "pointer-events-auto justify-center items-center bg-background pb-16" : "pointer-events-none justify-end items-center pb-[100px]"
    }`}>
      
      {step === "welcome" && (
        <div className="w-full max-w-2xl relative transition-all duration-500 flex flex-col items-center justify-center h-auto my-auto">
          {/* H:O Assistant Animated Character */}
          <motion.div 
            layout="position"
            initial={{ scale: 0.1, opacity: 0, y: 150, rotateZ: -10 }}
            animate="welcome"
            variants={{
              welcome: {
                scale: 1,
            ...
            onAnimationComplete={(definition) => {
              if (definition === "welcome") {
                window.dispatchEvent(new Event("resize"));
              }
            }}
            className="relative flex items-center justify-center transition-all duration-700 z-50 mb-8 h-96 w-96 sm:h-[30rem] sm:w-[30rem] md:h-[36rem] md:w-[36rem] lg:h-[44rem] lg:w-[44rem]"
          >
            ...
            <div className="w-full h-full relative overflow-visible">
               <Suspense fallback={
                 <Image 
                   ...
                   alt="H:O Assistant" 
                   fill
                   className="object-contain pointer-events-none drop-shadow-[0_20px_40px_rgba(var(--color-primary-rgb),0.5)]"
                   priority
                 />
               }>
                 <OnboardingRobotCanvas />
               </Suspense>
            </div>
          </motion.div>
        </div>
      ...
      <AnimatePresence mode="wait">
        {step === "welcome" && (
          <motion.div
            ...
            className="text-center space-y-6 relative z-10 w-full max-w-lg mx-auto"
          >
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight text-foreground">
                {locale === "lv" ? "Iepazīsimies?" : "Shall we get acquainted?"}
              </h1>
              
              <p className="text-lg sm:text-xl md:text-2xl text-foreground/80 leading-relaxed mx-auto">
                {locale === "lv" 
              ...
              <button
                ...
                className="mt-8 w-full rounded-2xl bg-primary px-8 py-5 text-lg sm:text-xl font-semibold text-background transition-transform active:scale-95 shadow-[0_0_20px_rgba(var(--color-primary-rgb),0.4)] hover:scale-105"
              >
          ...
          {step === "dashboard" && (
            <motion.div
              ...
              className="text-center space-y-3 bg-background/95 backdrop-blur-xl border border-border/50 p-6 rounded-3xl shadow-2xl relative w-full max-w-sm mx-auto pointer-events-auto"
            >
          ...
          {step === "reset_config" && (
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="text-center space-y-4 bg-background/95 backdrop-blur-xl border border-border/50 p-6 rounded-3xl shadow-2xl relative w-full max-w-sm mx-auto pointer-events-auto"
            >
