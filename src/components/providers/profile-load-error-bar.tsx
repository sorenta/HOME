...
export function ProfileLoadErrorBar() {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2 rounded-(--radius-card) border border-(--color-danger) bg-[color-mix(in_srgb,var(--color-surface)_92%,var(--color-danger))] px-3 py-2 text-sm text-(--color-text-primary) shadow-lg">
        <p className="font-medium">{t("auth.profile.loadFailed")}</p>
        <p className="break-words font-mono text-xs text-(--color-text-secondary)">
          {profileLoadError}
        </p>
        <button
          type="button"
          className="self-start rounded-(--radius-button) border border-(--color-border) bg-(--color-surface) px-3 py-1.5 text-xs font-semibold text-(--color-text-primary)"
