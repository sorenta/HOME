...
export function HouseholdSummary({
  ...
  if (!household) {
    return (
      <div className="maj-surface-panel text-sm text-(--color-text-secondary)">
        {t("household.loading")}
      </div>
    );
  }
  ...
  if (density === "compact") {
    return (
      <div className="maj-utility-strip relative z-10 rounded-(--radius-card) border border-[color-mix(in_srgb,var(--color-border)_75%,transparent)] bg-[color-mix(in_srgb,var(--color-surface)_82%,transparent)] px-3 py-2.5">
        <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <p className="min-w-0 truncate font-(family-name:--font-theme-display) text-base font-semibold text-(--color-text-primary)">
            {household.name}
          </p>
        ...
        <p className="mt-2 text-xs leading-snug text-(--color-text-secondary)">
  ...
  return (
    <div className="maj-glass-panel p-[length:var(--maj-space-card-pad)]">
      ...
      <div className="mt-4 flex flex-wrap gap-(--maj-space-stack)">
        <StatusPill tone="good">
          {memberCount} {t("household.members")}
        </StatusPill>
        <StatusPill>{t(plan === "premium" ? "billing.plan.premium" : "billing.plan.free")}</StatusPill>
      ...
      <p className="maj-theme-subtitle mt-3 text-sm text-(--color-text-secondary)">
        {t("household.shareHint")}
      </p>
      {showQr && inviteUrl ? (
        <div className="maj-nested-surface mt-4 p-[length:var(--maj-space-card-pad)]">
          ...
          <div className="mt-4 flex flex-col items-center gap-(--maj-space-stack)">
            <div className="rounded-(--radius-card) bg-(--color-card-elevated) p-3">
              <QRCodeSVG
                value={inviteUrl}
                size={168}
                bgColor="#ffffff"
            ...
            <button
              ...
              className="rounded-(--radius-button) border border-(--color-border) px-4 py-2 text-sm font-medium text-(--color-text-primary)"
            >
