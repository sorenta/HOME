...
export function HiveBackground({ beeCount = 5, navSelector, className }: Props) {
  ...
  const bees = useMemo(() => {
    return Array.from({ length: Math.max(1, Math.floor(effectiveCount)) }).map((_, i) => ({
      id: i,
      baseX: 8 + (i * (84 / Math.max(1, effectiveCount))), // percent of viewport width
      baseY: 12 + ((i * 11) % 62), // percent of viewport height
      ampX: 40 + Math.random() * 140,
      ampY: 18 + Math.random() * 72,
