...
export function SeasonalRewardModal() {
  ...
  return (
    <div className="fixed inset-x-4 bottom-24 z-50 mx-auto max-w-md rounded-3xl border border-[color:var(--color-surface-border)] bg-[color:var(--color-background)]/96 p-4 shadow-2xl backdrop-blur-xl">
      <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[color:var(--color-secondary)]">
        {t("seasonal.reward.unlocked")}
      </p>
      <h3 className="mt-2 font-[family-name:var(--font-theme-display)] text-xl font-semibold text-[color:var(--color-text)]">
      ...
      <button
        ...
        className="mt-4 w-full rounded-xl border border-[color:var(--color-primary)] bg-[color:var(--color-surface)] px-4 py-3 text-sm font-semibold text-[color:var(--color-text)]"
      >
