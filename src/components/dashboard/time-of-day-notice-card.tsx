...
export function TimeOfDayNoticeCard() {
  ...
  return (
    <motion.section
      ...
      className="maj-notice-strip relative z-10 rounded-(--radius-card) border border-[color-mix(in_srgb,var(--color-border)_70%,transparent)] bg-[color-mix(in_srgb,var(--color-surface)_78%,transparent)] px-3 py-2.5"
      aria-label={t("dashboard.timeNotice.aria")}
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
        <div className="min-w-0 flex-1">
          <p className="text-[0.62rem] font-bold uppercase tracking-[0.18em] text-(--color-text-secondary)">
            {t("dashboard.timeNotice.eyebrow")}
          </p>
          <p className="mt-0.5 text-sm font-semibold leading-snug text-(--color-text-primary)">
      ...
      {sleep ? (
        <p className="mt-2 border-t border-[color-mix(in_srgb,var(--color-border)_55%,transparent)] pt-2 text-xs text-(--color-text-secondary)">
