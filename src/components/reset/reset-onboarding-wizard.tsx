...
export function ResetOnboardingWizard({ initial, onComplete }: Props) {
  ...
  return (
    <GlassPanel className="space-y-6 border border-[color:var(--color-surface-border)] bg-[color:color-mix(in_srgb,var(--color-card)_92%,white_8%)] p-4 sm:p-5">
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <p className="text-sm font-semibold text-[color:var(--color-text-secondary)]">
          {tr("reset.onboarding.flow", "RESET anketa")}
          </p>
          <span className="rounded-full border border-[color:var(--color-surface-border)] bg-[color:var(--color-surface)] px-3 py-1 text-xs font-semibold text-[color:var(--color-text-secondary)]">
            {stepLabel}
          </span>
        </div>
      ...
      <AnimatePresence mode="wait" initial={false}>
              ...
              {goalOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      "rounded-[var(--radius-card)] border px-4 py-3 text-left transition",
                      active
                  ...
                  >
                    <p className="text-sm font-semibold text-[color:var(--color-text-primary)]">{option.label}</p>
                    {option.hint && (
                      <p className="mt-1 text-xs text-[color:var(--color-text-secondary)]">{option.hint}</p>
                    )}
                  </button>
                );
              })}
        ...
        {step === 1 && (
          ...
          >
            <div className="space-y-3">
              <div className="space-y-2">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[color:var(--color-text-secondary)]">
                    ...
                    return (
                      <button
                        ...
                        className={[
                          ...
                          active
                            ? "border-[color:var(--color-accent)] bg-[color:var(--color-surface-2)] text-[color:var(--color-text-primary)]"
                            : "border-[color:var(--color-surface-border)] bg-[color:var(--color-surface)] text-[color:var(--color-text-secondary)]",
                        ].join(" ")}
                      >
                        {option.label}
                      </button>
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  className="w-full rounded-[var(--radius-card)] border border-[color:var(--color-surface-border)] bg-[color:var(--color-surface)] px-3 py-2 text-sm text-[color:var(--color-text-primary)]"
                >
              ...
              {frequencyOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      "rounded-[var(--radius-card)] border px-4 py-3 text-sm transition",
                      active
                        ? "border-[color:var(--color-accent)] bg-[color:var(--color-surface-2)] text-[color:var(--color-text-primary)]"
                        : "border-[color:var(--color-surface-border)] bg-[color:var(--color-surface)] text-[color:var(--color-text-secondary)]",
                    ].join(" ")}
            ...
            <div className="space-y-3">
              ...
              {answers.quitPlan && (
                <div className="grid gap-3 sm:grid-cols-2">
                  <label className="space-y-1 text-sm text-[color:var(--color-text-secondary)]">
                    ...
                    <input
                      ...
                      className="w-full rounded-[var(--radius-card)] border border-[color:var(--color-surface-border)] bg-[color:var(--color-surface)] px-3 py-2 text-[color:var(--color-text-primary)]"
                    />
                  </label>
                  <label className="space-y-1 text-sm text-[color:var(--color-text-secondary)]">
      ...
      <div className="flex items-center justify-between gap-3 border-t border-[color:var(--color-surface-border)] pt-4">
        <button
          type="button"
          onClick={() => setStep((prev) => Math.max(0, prev - 1))}
          disabled={step === 0}
          className="rounded-full border border-[color:var(--color-surface-border)] bg-[color:var(--color-surface)] px-4 py-2 text-sm text-[color:var(--color-text-secondary)] disabled:opacity-40"
        >
        ...
        {step < totalSteps - 1 ? (
          <button
            ...
            className="rounded-full border border-[color:var(--color-accent)] bg-[color:var(--color-surface-2)] px-4 py-2 text-sm font-semibold text-[color:var(--color-text-primary)] disabled:opacity-40"
          >
