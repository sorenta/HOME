...
function WeighSparkline({ entries }: { entries: WeighInEntry[] }) {
  ...
  return (
    <svg
      viewBox={`0 0 ${w} ${h}`}
      className="mt-2 w-full max-w-full text-[color:var(--color-primary)]"
      aria-hidden
    >
      <polyline
        fill="none"
...
export function ResetBodyTracking({ state, onUpdate }: Props) {
  ...
  return (
    <GlassPanel className="space-y-4">
      ...
      <div className="rounded-2xl border border-[color:var(--color-surface-border)] bg-[color:var(--color-surface)]/35 p-4">
        ...
        <div className="mt-2 flex flex-wrap items-end gap-2">
          <input
            ...
            className="min-w-[6rem] flex-1 rounded-xl border border-[color:var(--color-surface-border)] bg-[color:var(--color-background)] px-3 py-2 text-sm text-[color:var(--color-text)]"
          />
          <button
            type="button"
            onClick={addWeighIn}
            className="rounded-xl bg-[color:var(--color-primary)] px-4 py-2 text-sm font-semibold text-[color:var(--color-background)]"
          >
        ...
        {weighSorted.length >= 2 ? (
          <div className="mt-3">
            <p className="text-xs uppercase tracking-[0.15em] text-[color:var(--color-secondary)]">
              {t("reset.wellness.body.trend")}
            </p>
            <WeighSparkline entries={state.weighIns} />
          </div>
        ) : (
          <p className="mt-3 text-xs text-[color:var(--color-secondary)]">
            {t("reset.wellness.body.trendNeedTwo")}
          </p>
        )}
        {weighSorted.length > 0 ? (
          <ul className="mt-2 space-y-1 text-xs text-[color:var(--color-secondary)]">
            {weighSorted
              .slice(-5)
              .reverse()
              .map((e) => (
      ...
      <div className="rounded-2xl border border-[color:var(--color-surface-border)] bg-[color:var(--color-surface)]/35 p-4">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <select
            ...
            className="rounded-xl border border-[color:var(--color-surface-border)] bg-[color:var(--color-background)] px-3 py-2 text-sm text-[color:var(--color-text)]"
          >
            {AREAS.map((a) => (
          ...
          <input
            ...
            className="min-w-[5rem] flex-1 rounded-xl border border-[color:var(--color-surface-border)] bg-[color:var(--color-background)] px-3 py-2 text-sm text-[color:var(--color-text)]"
          />
          <button
            type="button"
            onClick={addMeasurement}
            className="rounded-xl border border-[color:var(--color-surface-border)] px-4 py-2 text-sm font-semibold text-[color:var(--color-text)]"
          >
            {t("reset.wellness.body.addMeasure")}
          </button>
        </div>
        {recentMeasures.length > 0 ? (
          <ul className="mt-3 space-y-1 text-sm text-[color:var(--color-text)]">
