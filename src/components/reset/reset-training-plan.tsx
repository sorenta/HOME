...
export function ResetTrainingPlan({ mode, state, onUpdate }: Props) {
  ...
  return (
    <GlassPanel className="space-y-4">
      ...
      <p className="text-sm text-(--color-secondary)">{t("reset.training.hint")}</p>

      <div className="flex items-center justify-between gap-2 rounded-2xl border border-(--color-surface-border) bg-(--color-surface)/35 px-3 py-2">
        <button
          type="button"
          onClick={prevWeek}
      ...
      <ol className="space-y-2">
        {week.map((slot, i) => (
          <li
            key={slot.titleKey}
            className="rounded-2xl border border-(--color-surface-border) bg-(--color-surface)/25 px-3 py-3"
          >
            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-(--color-secondary)">
              {t(DAY_INDEX_KEYS[i])}
            </p>
            <p className="mt-1 text-sm font-semibold text-(--color-text)">
              {t(slot.titleKey)}
            </p>
            <p className="mt-1 text-sm leading-relaxed text-(--color-secondary)">
              {t(slot.detailKey)}
            </p>
          </li>
        ))}
