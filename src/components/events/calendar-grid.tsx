...
import { useEventsCalendarTheme } from "@/components/events/events-theme-layer";
...
export function CalendarGrid({
  ...
  const { dayCell, selectedCell, eventDot } = useEventsCalendarTheme();
  const { t } = useI18n();
    ...
    <section className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <button
          ...
          aria-label={t("events.calendar.prevMonth")}
        >
          ←
        </button>
        ...
        {monthDays.map((day, index) => {
          ...
          return (
            <button
              ...
              className={`aspect-square p-2 text-left transition-transform active:scale-[0.98] ${isSelected ? selectedCell : dayCell}`}
              style={{
                borderRadius: "var(--radius-md)",
                  ...
                  <div className="mt-auto flex items-center gap-1">
                    {Array.from({ length: Math.min(indicatorCount, 3) }).map((_, dotIndex) => (
                      <span
                        ...
                        className={`h-1.5 w-1.5 ${eventDot}`}
                        style={{
                          borderRadius: "999px",
